...
bool isHeapUtil(struct Node1* root1){
   ...
   if (root1->right == NULL){
      ...
      if (root1->key >= root1->left->key && root1->key >= root1->right->key)
      return ((isHeapUtil(root1->left)) && (isHeapUtil(root1->right)));
      else
         return (false);
   }
}
bool isHeap(struct Node1* root1){
   ...
   if (isCompleteUtil(root1, index1, node_count) && isHeapUtil(root1))
   return true;
   return false;
}
// Driver program
